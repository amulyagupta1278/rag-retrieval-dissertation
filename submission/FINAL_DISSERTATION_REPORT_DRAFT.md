...
An exploratory Phase 8 R4 scaling study expanded the corpus to 130 documents and 954 chunks and evaluated 100 inherited questions, with 20 synthesis candidates held outside primary metrics. The owner reviewed all 140 crosswalk mappings, 100 generated answers, and 20 synthesis candidates. One inherited mapping was intentionally graded nonrelevant. On the locked 40-question test split, Prompt-RAG achieved the highest Recall@10 (0.8875) and nDCG@10 (0.6469), while BM25 achieved the highest MRR@10 (0.6071). R4 therefore provides human-owner-validated exploratory scaling evidence; it does not supersede the preregistered pilot. Results support evaluating retrieval and generation as separate layers.
...
Confirmatory claims apply to the frozen V2 pilot: 22 documents, 140 chunks, 34 questions, and five retrieval systems. Phase 8 R4 is reported separately as a human-owner-validated exploratory scaling study with 130 documents, 954 chunks, 100 primary questions, and 20 separately reviewed synthesis candidates. Owner review covers all 140 inherited mappings, 100 generated answers, and 20 synthesis decisions; this validation improves R4's evidential quality but does not make its non-preregistered results confirmatory. Prompt-RAG is a reranker rather than the downstream answer generator. In the pilot it reranks BM25's top 50; in R4 it reranks a deterministic BM25-top-25 plus unseen-FAISS-top-25 union. The graph systems are entity-co-occurrence retrievers, not Microsoft GraphRAG.
...
R4 expands the corpus from 22 to 130 documents and from 140 to 954 section-aware chunks. It retains 100 inherited primary questions across exact lookup, terminology, paraphrase, entity relation, and multi-hop categories. Sixty questions form a development split and 40 form a locked test split. Twenty deterministic synthesis questions remain outside primary metrics. Owner review accepted 12, marked four for revision, and rejected four.
...
All 140 R4 relevance crosswalks received owner review. One preserved mapping was graded 0 because the source chunk did not support the claimed fact; 139 received grade 2. Statistical comparisons remain explicitly exploratory because R4 design and hypotheses were not preregistered and owner review was single-rater.
...
|---|---:|---:|---:|---:|
| BM25 | **0.6071** | 0.8625 | 0.1200 | 0.6460 |
| FAISS-windowed-max | 0.4113 | 0.6750 | 0.0975 | 0.4451 |
| Entity Graph v4 | 0.4579 | 0.6750 | 0.0950 | 0.4724 |
| Hybrid RRF | 0.5938 | 0.8625 | 0.1200 | 0.6306 |
| Prompt-RAG Claude | 0.5842 | **0.8875** | **0.1225** | **0.6469** |

Expansion changed the ordering seen in the pilot. BM25 led R4 locked-test MRR, Prompt-RAG led recall, precision, hit rate, and nDCG, and Hybrid remained close to both. Prompt-RAG mean latency was 16.82 seconds (median 15.80; p95 24.25). Comparable latency was unavailable for the other systems and was not encoded as zero.

Exploratory paired nDCG@10 tests used 10,000 sign-flip randomizations, paired bootstrap intervals, seed 42, and Holm correction. BM25, Hybrid, and Prompt-RAG each outperformed FAISS after correction (adjusted p-values 0.0027, 0.0020, and 0.0056 respectively). Other adjusted comparisons were not below 0.05. These tests do not prove preregistered hypotheses because R4 was not the confirmatory benchmark.
...
## 5.5 Phase 8 R4 generation and owner evaluation
...
The owner reviewed all 100 R4 answers. Mean correctness scores were 1.3 for BM25, 1.6 for FAISS, 1.8 for Graph, 1.4 for Hybrid, and 1.5 for Prompt-RAG. Mean completeness was 1.0, 1.1, 1.1, 1.0, and 1.2 respectively. Faithfulness and citation accuracy were constant at 2.0 and unsupported-claim severity at 0, preventing meaningful between-system inference for those dimensions. Prior AI labels remain preserved for disclosed owner–AI comparison but are not used as final R4 labels.

R4 H5 analysis remained descriptive. MRR–faithfulness was non-estimable because faithfulness was constant. MRR–correctness had Spearman rho -0.1888 with interval [-0.4009, 0.0698]; nDCG–correctness rho -0.1659 [-0.3956, 0.0880]; and recall–completeness rho 0.0651 [-0.2746, 0.4232]. None supports a strong monotonic relationship between retrieval metrics and owner-reviewed answer quality.
...
Pooled judgments are incomplete: unpooled chunks are not independently verified nonrelevant. Complete Evidence Recall depends on judged positives, not unknowable total corpus evidence. Pilot generation labels remain mostly automated; R4 generation labels are owner-reviewed but single-rater and strongly imbalanced.

## 7.3 External validity

Twenty-two documents and 34 questions cannot represent every government scheme, language, user population, or information need. R4 materially broadens engineering coverage to 130 documents and 954 chunks, but it reuses inherited questions and owner-reviewed automatic crosswalk candidates. It therefore reduces corpus-scale uncertainty without establishing population-level validity. Neither study covers multilingual questions, diverse user groups, or independently sampled real-world information needs.
...
R4 shows that the pipeline can scale from 22 to 130 documents and from 140 to 954 chunks while rebuilding all five systems, enforcing cost and failure controls, and completing retrieval, generation, and owner review. Its changed system ordering is itself important: apparent winners depend on benchmark scale and construction. Pilot remains canonical confirmatory basis; R4 adds human-owner-validated exploratory scale evidence.

Future work should revise or replace eight nonaccepted synthesis candidates, conduct independent multi-rater replication, improve graph entity linking, test multilingual questions, and evaluate partial-answer abstention policies. R4 should remain separate from preregistered pilot claims despite completed owner review.
...
- Phase 8 R4 owner-validation report: `docs/PHASE8_R4_HUMAN_VALIDATION_FINAL_REPORT.md`
- Phase 8 R4 owner-validated status: `audits/phase8_r4_human_validated/canonical_status.json`
