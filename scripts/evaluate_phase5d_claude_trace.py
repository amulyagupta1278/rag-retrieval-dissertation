#!/usr/bin/env python3
"""Evaluate Claude trace repeatability only; never read relevance data."""
...
from scripts.run_phase5d_prompt_rag_claude import (  # noqa: E402
    OUTPUT_ROOT,
    TRACE_DECISION_PATH,
    build_logical_plan,
    preflight,
...
def _json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ClaudeContractError(f"invalid trace artifact: {path}") from exc
    if not isinstance(value, dict):
        raise ClaudeContractError(f"trace artifact must be object: {path}")
    return value
...
def evaluate() -> dict[str, Any]:
    """Revalidate exact 24 records and calculate no relevance metrics."""
    ...
    if ledger.get("status") != "complete" or ledger.get(
        ...
        raise ClaudeContractError("trace ledger is not complete exact 24-call scope")
    plan = build_logical_plan("trace", frozen["query_ids"], frozen["trace_ids"])
...
def main() -> int:
    try:
        ...
        if TRACE_DECISION_PATH.exists():
            raise ClaudeContractError("trace decision exists; refusing overwrite")
