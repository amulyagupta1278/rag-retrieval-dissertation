...
def bootstrap(a,b,metric,seed=SEED,n=BOOT):
 if not a or not b or len(a)!=len(b): raise ValueError('paired bootstrap requires equal nonzero lengths')
 left=[x.get('query_id') for x in a]; right=[x.get('query_id') for x in b]
 if any(not isinstance(x,str) or not x for x in left+right): raise ValueError('paired bootstrap requires query_id on every row')
 if len(set(left))!=len(left) or len(set(right))!=len(right): raise ValueError('paired bootstrap rejects duplicate query IDs')
 if left!=right: raise ValueError('paired bootstrap requires exact identical query-ID order')
...
def main():
 ...
 h1cats={'exact_lookup','terminology'}; h1a=[x for x in evals['bm25'] if x['category'] in h1cats]; h1b=[x for x in evals['faiss'] if x['category'] in h1cats]; h1=bootstrap(h1a,h1b,'mrr_at_10'); h1.update({'bm25_known_gold_mrr_at_10':statistics.mean(x['metrics']['mrr_at_10'] for x in h1a),'faiss_known_gold_mrr_at_10':statistics.mean(x['metrics']['mrr_at_10'] for x in h1b),'primary_margin':[-.05,.05],'equivalence_decision':'equivalent' if h1['ci95'][0]>-.05 and h1['ci95'][1]<.05 else 'inconclusive','sensitivity_margin':[-.03,.03],'sensitivity_equivalent':h1['ci95'][0]>-.03 and h1['ci95'][1]<.03,'scope':'exact_lookup + terminology only','inference':'exploratory pilot'})
