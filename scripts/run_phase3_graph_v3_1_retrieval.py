#!/usr/bin/env python3
"""Run frozen Graph v3.1 retrieval only; never read relevance/evaluation data."""
...
APPROVED = {
    ...
    "chunk_entities": (ROOT / "runs/v2/phase3_graph_v3_1/index/chunk_entities.json").resolve(),
    "freeze_manifest": (ROOT / "runs/v2/phase3_graph_v3_1/index_freeze_manifest.json").resolve(),
    "queries": (ROOT / "runs/v2/phase3_graph_v3_1/inputs/r5_queries_only.jsonl").resolve(),
    "output": (ROOT / "runs/v2/phase3_graph_v3_1").resolve(),
}
...
def main() -> None:
    ...
    output_files = [
        paths["output"] / "rankings/graph_v3_1_top50.jsonl",
        paths["output"] / "rankings/graph_v3_1_top10.jsonl",
        paths["output"] / "traces/graph_v3_1_full_traces.jsonl",
    ...
    if freeze["status"] != "entity_graph_v3.1_frozen_before_queries" or freeze["benchmark_queries_run"] != 0:
    ...
    write_jsonl(paths["output"] / "rankings/graph_v3_1_top50.jsonl", top50_rows, key="query_id", overwrite=args.overwrite)
    write_jsonl(paths["output"] / "rankings/graph_v3_1_top10.jsonl", top10_rows, key="query_id", overwrite=args.overwrite)
    write_jsonl(paths["output"] / "traces/graph_v3_1_full_traces.jsonl", traces, key="query_id", overwrite=args.overwrite)
