...
def _metrics(runs: list[dict], qrels: dict[str, dict[str, int]]) -> dict:
    ...
    for run in runs:
        ...
        gold = {
            chunk_id for chunk_id, relevance in qrels.get(run["query_id"], {}).items()
            if relevance > 0
        }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--qa", default="data/queries/qa_dataset.jsonl")
    parser.add_argument("--qrels", default="data/qrels/qrels.tsv")
...
def main() -> None:
    ...
    chunk_ids = {item["chunk_id"] for item in load_jsonl(args.chunks)}
    aggregate: dict[str, dict] = {}
    per_category: dict[str, dict[str, dict]] = {}
    integrity: dict[str, dict] = {}

    for system in SYSTEMS:
        runs = load_jsonl(Path(args.runs_root) / "retrieval" / f"{system}_run.jsonl")
    ...
    output = output_root / "reports" / "phase0_benchmark_metrics.json"
    ...
    with (output_root / "metrics" / "phase0_benchmark_metrics.csv").open("w", encoding="utf-8", newline="") as handle:
    ...
    for system in SYSTEMS:
        ...
        lines.append(f"| {DISPLAY_NAMES[system]} | {row['mrr_at_5']:.4f} | {row['mrr_at_10']:.4f} | {row['recall_at_5']:.4f} | {row['recall_at_10']:.4f} | {row['ndcg_at_10']:.4f} | {row['avg_latency_ms']:.2f} |")
    ...
    for system in SYSTEMS:
        for category in CATEGORIES:
            ...
            lines.append(f"| {DISPLAY_NAMES[system]} | {category} | {row['mrr_at_5']:.4f} | {row['mrr_at_10']:.4f} | {row['recall_at_5']:.4f} | {row['recall_at_10']:.4f} | {row['ndcg_at_10']:.4f} | {row['avg_latency_ms']:.2f} |")
    lines += ["", "## Sanity flags", "", *(f"- {flag}" for flag in flags or ["None"])]
    (output_root / "reports" / "phase0_benchmark_metrics.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
