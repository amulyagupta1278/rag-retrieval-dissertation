#!/usr/bin/env python3
"""Audit exactly eight frozen v3.2 traces without reading qrels or metrics."""
...
def main() -> None:
    ...
    approved = {
        ...
        "traces": (ROOT / "runs/v2/phase3_graph_v3_2/traces/graph_v3_2_full_traces.jsonl").resolve(),
        "retrieval_manifest": (ROOT / "runs/v2/phase3_graph_v3_2/retrieval_manifest.json").resolve(),
        "output": (ROOT / "audits/phase3_graph_v3_2/eight_trace_audit.jsonl").resolve(),
        "decision": (ROOT / "audits/phase3_graph_v3_2/trace_validity_decision.json").resolve(),
    }
    if paths != approved:
    ...
    for query_id, expected in sorted(SELECTED.items()):
        ...
        if query_id == "v2q-029":
            provenance = [
                item for item in (expected_seed or {}).get("alias_provenance", [])
                if normalize_text(str(item["alias"])) == "ujjwala 2 0"
            ]
            alias_provenance_valid = (
                len(provenance) == 1
                and provenance[0]["evidence_source"] == "frozen chunks only; no benchmark inputs"
                and bool(provenance[0]["evidence_chunk_ids"])
            )
        checks = {
            ...
            "corpus_backed_alias_provenance_valid": alias_provenance_valid,
        }
    ...
    decision = {
        "status": "invalid_v3_2_trace_gate" if invalid_rows else "passed_trace_validity_gate",
        ...
        "failure": {"invalid_query_ids": [row["query_id"] for row in invalid_rows], "failed_checks": {row["query_id"]: row["failed_checks"] for row in invalid_rows}} if invalid_rows else None,
        "required_next_step": "stop with exact defect; do not evaluate or expand pool" if invalid_rows else "stop for owner trace approval before evaluation",
