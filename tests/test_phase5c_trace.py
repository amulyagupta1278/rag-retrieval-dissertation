...
def test_phase5c_freeze_manifest_is_complete_and_exact() -> None:
    ...
    assert len(manifest["artifact_hashes"]) == 17
...
def test_failed_v2_attempt_is_preserved_and_api_version_correction_is_narrow() -> None:
    correction = json.loads(
        (ROOT / "audits/phase5c/api_version_validity_correction.json").read_text(
            encoding="utf-8"
        )
    )
    evidence = correction["evidence"]
    assert sha256(ROOT / evidence["api_version_404_checkpoint_path"]) == evidence[
        "api_version_404_checkpoint_sha256"
    ]
    assert evidence["invalid_v2_successful_response_n"] == 0
    corrected = correction["corrected_contract"]
    assert corrected["api_version_before"] == "v1"
    assert corrected["api_version_after"] == "v1beta"
    for unchanged in (
        "candidate_set_changed",
        "model_changed",
        "prompt_changed",
        "ranking_or_score_contract_changed",
        "response_schema_changed",
        "trace_selection_changed",
    ):
        assert corrected[unchanged] is False
    assert correction["runtime_gate"]["live_execution_authorized_by_this_artifact"] is False
